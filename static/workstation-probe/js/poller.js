...
//
// When cfg.api === 'sample-stream', the poller bypasses the network
// entirely and uses the local sample-stream.js generator that produces
// a 60-second loop of synthetic data matching a 32-core / 512 GB / 1 GPU
// / 1 TB disk workstation.

import { generateMetrics, generateProfile } from './sample-stream.js';
...
const SAMPLE_API = 'sample-stream';
...
export function subscribe(cfg, callback) {
  ...
  if (!poller) {
    poller = cfg.api === SAMPLE_API ? createSamplePoller(cfg) : createPoller(cfg);
...
// createSamplePoller returns a poller that uses the local sample-stream
// generator. No network I/O; profile is loaded synchronously and tick()
// calls generateMetrics on every interval.
function createSamplePoller(cfg) {
  const poller = {
    cfg,
    callbacks: new Set(),
    timerId: null,
  ...
  // Load profile synchronously (no network).
  try {
    poller.profile = generateProfile();
  } catch (e) {
  ...
  function tick() {
    ...
    try {
      const now = Date.now();
      poller.lastData = generateMetrics(now);
      poller.lastError = null;
      poller.hasSuccess = true;
      poller.lastSuccessAt = now;
  ...
  function scheduleNext() {
    poller.timerId = setTimeout(tick, intervalMs);
  }
